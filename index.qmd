...
# Introductory training to `Python` in SSP
...
The different chapters can be launched with a single click from the [training space](https://www.sspcloud.fr/formation?search=&path=%5B%22Initiation%20%C3%A0%20Python%22%5D) of the `SSP Cloud`. Launching a chapter means launching a `jupyter-python` service on the `SSP Cloud`, pre-configured for the training and which will open directly on the notebook associated with the chapter.

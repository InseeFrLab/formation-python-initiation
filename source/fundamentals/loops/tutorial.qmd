...
Que se passe-t-il si l'on oublie d'incrémenter `i` ? Le critère d'arrêt n'est jamais atteint, la boucle est donc infinie, et il faut arrêter manuellement la cellule en cours. Lorsqu'on pressent qu'une boucle `while` met trop longtemps à tourner, il faut considérer l'hypothèse que l'on soit tombé dans une boucle infinie, et bien vérifier que le critère d'arrêt est atteignable.

Par exemple, le code ci-dessous (commenté pour ne pas empêcher l'exécution) génère une boucle infinie car le critère d'arrêt n'est jamais atteint.

...
Illustrons l'installation de package à travers le package [emoji](https://pypi.org/project/emoji/), qui permet de représenter des émoticônes dans les sorties de `Python`. Pour le coup, celui-ci n'est pas encore installé, il n'est donc pas encore possible de l'importer.

```{python}
import emoji
```

Pour installer un package, la commande est simple : `pip install nom_du_package`. Sans rentrer dans les détails, `pip` est un logiciel (installé avec `Python`) qui s'utilise en ligne de commande (dans un terminal). Pour pouvoir envoyer une commande au terminal depuis un notebook Jupyter, on rajoute un `!` devant la commande.
...
Dans un environnement de notebook `Jupyter`, le code `Python` est exécuté de manière **interactive**, cellule par cellule. Cependant, en dehors de `Jupyter`, le code est généralement écrit et exécuté sous forme de scripts. Un script `Python` est simplement un fichier texte portant l'extension `.py` et qui contient une série d'instructions `Python` qui vont être exécutées linéairement par l'interpréteur `Python`.
...
Le fichier `output_script.txt` a bien été créé en local et le message attendu a été imprimé dans la sortie de la console.
...
La fonction contenue dans ce module peut être importée comme nous l'avons vu dans ce tutoriel. Notons que le module doit lui même importer les packages/modules nécessaires au bon fonctionnement des fonctions qu'il contient. 
...
Dans la cellule précédente, le fichier `normalisation.py` était importé comme un module. Ainsi, la variable `__name__` vaut `normalisation` et c'est pourquoi le code sous la condition `if` n'a pas été exécuté. Lorsque le fichier est exécuté comme script, ce code est exécuté.

...
Polars propose tout une série de méthodes, appelées `selectors`, très pratique pour effectuer des opérations sur des colonnes (sélection, mutation etc) en fonction de leur nom ou de leur type.
...
L'habitude est d'importer les selectors avec `import polars.selectors as cs`.
...
Des explications détaillées sont disponibles dans la [documentation officielle](https://docs.pola.rs/api/python/stable/reference/selectors.html).
...
Les méthodes comme .drop_nulls() ou .fill_null() permettent de gérer explicitement ces valeurs.

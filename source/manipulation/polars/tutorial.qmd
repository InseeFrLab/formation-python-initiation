...
- Polars permet de traiter des données _out of memory_ : `Polars` présente aussi l'avantage
...
### Quelques fonctionnalités encore instables
Les catégories de Polars ne sont pas encore très bien.
On a tendance à traiter ces variable catégorielles comme des caractères.

### Pas (encore) de `geopolars`
A l'inverse, Polars est encore relativement récent.
...
### `Polars` reste moins répandu que `pandas`
Polars étant plus récent, il est en général moins bien interfacé avec les autres
...
## Caractéristiques de polars pour commencer
Polars présente plusieurs caractéristiques à garder en tête.

### Petits conseils pour commencer
- Import plus délicat
Polars est très strict sur les types des colonnes quand on importe des données.
Il est courant de passer du temps à débugger cette étape.
Pour optimiser ce temps là,

-  Colonnes sont associées à des string
En Polars, les caractères simples `""`

- l'index de pandas est mort <3

...
```{python}
print(df.select("var1", "experiment"))  # équivalent
...
```{python}
print(df.filter(pl.col("var1")>=0))  # équivalent à
...
En regardant le message d'erreur, polars traite la colonne `annais` comme un entier.
...
On peut aussi utiliser les `selectors` de polars pour faire des opérations groupées sur un ensemble
de colonnes.
Il suffit de remplacer le `pl.col("mycol")` par le sélecteur recherché.
Pour modifier les noms en série, on peut utiliser les fonctions liées aux méthodes `Expr.name`, *cf.* [documentation officielle](https://docs.pola.rs/api/python/stable/reference/expressions/name.html).

Par exemple, si l'on veut normaliser à 1 toutes les colonnes numériques, on peut le faire en une ligne en spécifiant uniquement la colonne de référence :

```{python}
df.with_columns(
    (cs.numeric() / pl.col("var2")).name.suffix("_norm")
)
```
...
```{python}
print(df.filter(pl.col("enseigne") == 'Carrefour'))
```
...
```{python}
print(df.filter(pl.col("quantite") > 5))
```
...
```{python}
print(df.filter(pl.col("date_heure").dt.hour() > 15))
```
...
```{python}
print(df.filter(pl.col("date_heure").dt.date() == pl.Series(['2022-01-03']).str.to_date()))
```

...
- la clé de jointure. Par défaut, Pandas essaie de joindre les deux DataFrame à partir de leurs index. En pratique, on spécifie souvent une variable présente dans le DataFrame comme clé de jointure (paramètre `on` si la variable porte le même nom dans les deux DataFrame, ou `left_on` et `right_on` sinon).
...
La jointure de type `inner` est la plus intuitive : elle ne crée généralement pas de valeurs manquantes et permet donc de travailler directement sur la table fusionnée. Mais attention : si beaucoup de clés ne sont pas présentes dans les deux DataFrame à la fois, une jointure `inner` peut aboutit à des pertes importantes de données, et donc à des résultats finaux biaisés. Dans ce cas, il vaut mieux choisir une jointure à gauche ou à droite, selon la source que l'on cherche à enrichir et pour laquelle il est donc le plus important de limiter les pertes de données.
:::
...
### Exercice : sélection de données dans un DataFrame Pandas
...
### Exercice : exploration du fichier des prénoms

Le [fichier des prénoms](https://www.insee.fr/fr/statistiques/2540004?sommaire=4767262) contient des données sur les prénoms attribués aux enfants nés en France entre 1900 et 2021. Ces données sont disponibles au niveau France, par département et par région. L'objectif de ce tutoriel est de proposer une analyse de ce fichier, du nettoyage des données au statistiques sur les prénoms.
...
### Exercice : calcul d'une empreinte carbone par habitant au niveau communal
...
Voici un exercice pour pratiquer le traitement des données temporelles avec Pandas, notamment pour calculer les variations d'un indice macroéconomique d'un trimestre à l'autre.

### Exercice : Évolution d'un Indice Macroéconomique Trimestriel

Vous disposez d'un fichier CSV nommé `indice_macro.csv` qui contient deux colonnes : `Date` et `Indice`. La colonne `Date` est formatée en trimestres (`YYYY-QQ`), et la colonne `Indice` représente la valeur d'un certain indice macroéconomique.

#### Objectifs :
- Importez le fichier CSV dans un DataFrame Pandas.
- Convertissez la colonne `Date` au format datetime approprié pour des données trimestrielles.
- Calculez l'évolution de l'indice d'un trimestre à l'autre.
- Calculez l'évolution glissante de l'indice, par exemple sur un glissement de 4 trimestres.

#### Étapes :

1. **Importation et préparation des données**
   - Importez les données à partir du fichier CSV.
   - Convertissez la colonne `Date` en type `datetime` adapté aux données trimestrielles.

2. **Calcul de l'évolution trimestrielle**
   - Utilisez la méthode `.shift()` pour créer une nouvelle colonne qui contiendra l'indice du trimestre précédent.
   - Calculez la différence entre l'indice actuel et l'indice décalé pour obtenir l'évolution d'un trimestre à l'autre.

3. **Calcul de l'évolution glissante**
   - Utilisez la méthode `.rolling()` pour appliquer une fenêtre glissante et `.sum()` ou une autre fonction d'agrégation pour calculer l'évolution sur la période souhaitée.

#### Code pour tester vos réponses :

```python
# Testez votre code dans cette cellule
```
...
Les étudiants peuvent utiliser cet exercice pour pratiquer la manipulation des dates et des calculs d'évolution dans Pandas, des compétences essentielles pour l'analyse de séries temporelles économiques.

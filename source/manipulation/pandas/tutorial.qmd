...
### Travailler avec des données tabulaires
...
### Exporter un DataFrame au format CSV
...
#### Importer des données à partir d'un fichier Parquet

Le format Parquet est un autre format pour le stockage de données tabulaires, de plus en plus fréquemment utilisé. Sans rentrer dans les détails techniques, le format Parquet présente différentes caractéristiques qui en font un choix privilégié pour le stockage et le traitement de gros volumes de données. En raison de ces avantages, le format Parquet est de plus en plus utilisé comme format de mise à disposition des données à l'Insee. Il est donc essentiel de savoir importer et requêter des fichiers Parquet avec Pandas.

Pour lire un fichier Parquet dans un DataFrame pandas, on utilise la fonction `read_parquet()`.

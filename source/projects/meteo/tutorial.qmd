...
- convertir la colonne de temps au format `datetime` ([documentation](https://pandas.pydata.org/docs/reference/api/pandas.to_datetime.html))
...
```{python}
solutions.plot_daily_avg_bhi(df_preds)
```

#### À vous de jouer !

```{python}
def plot_daily_avg_bhi(df_preds):
    # Votre code ici
    return None
```

```{python}
plot_daily_avg_bhi(df_preds)
```
...
```{python}
solutions.plot_hourly_avg_bhi(df_preds)
```

#### À vous de jouer !

```{python}
def plot_hourly_avg_bhi(df_preds):
    # Votre code ici
    return None
```

```{python}
plot_hourly_avg_bhi(df_preds)
```
